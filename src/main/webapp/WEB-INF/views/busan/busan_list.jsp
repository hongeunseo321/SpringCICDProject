		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-6" v-for="vo in store.list">
				<div class="blog__item">
					...
					<div class="blog__item__text">
						...
						<p>{{vo.address}}</p>
						<a :href="'http://localhost:8080/busan/detail?no='+vo.no+'&type='+store.type" class="blog__btn">READ MORE <span
			...
			<div class="col-lg-12">
				<div class="product__pagination blog__pagination">
					<a v-if="store.startPage>1" @click="store.prev(store.startPage-1)"><i class="fa fa-long-arrow-left" style="cursor: pointer;"></i></a>
					<a v-for="i in store.range(store.startPage,store.endPage)" @click="store.pageChange(i)" style="cursor: pointer;">{{i}}</a> 
					<a v-if="store.endPage<store.totalpage" @click="store.next(store.endPage+1)"><i class="fa fa-long-arrow-right"  style="cursor: pointer;"></i></a>
				</div>
			</div>
		</div>
	</div>
